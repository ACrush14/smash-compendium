...
async function main() {
  const f = await db.fighter.findFirst({
    where: { name: "Greninja" },
    select: {
      id: true, name: true, curationStatus: true,
      ...
      tips: { select: { id: true, titleEn: true, titleJp: true, textJp: true }, orderBy: { id: "asc" } },
    },
  });
  if (!f) { console.log("Greninja not found"); return; }
  console.log(`=== ${f.name} (${f.id}) status=${f.curationStatus} ===`);
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Greninja" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Greninja": ${orphans.length}`);
