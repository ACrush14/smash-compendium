...
async function translateText(text: string, sl = 'auto', tl = 'pt') {
...
async function fetchWikiText(pageTitle: string, wikiUrl = "https://www.ssbwiki.com") {
  try {
...
async function main() {
  ...
  for (const f of ssb64Roster) {
    ...
    // Usually under "== ゲーム中の解説 ==" or "== 概要 =="
    let bioJp = "";
    const matchJp = jpText.match(/==\s*ゲーム中の解説\s*==\n====.*?スマブラ64.*?\n.*?紹介画面.*?\'\'\'\n(.*?)(?=\n====|\n===|\n==)/is);
    if (matchJp) {
      bioJp = matchJp[1].replace(/<br>/gi, "\n").trim();
    } else {
      // Fallback: Just grab the first paragraph under 概要
