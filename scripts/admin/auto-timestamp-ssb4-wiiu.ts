...
const VIDEO_PATH = 'full_video_ssb4_wiiu.mp4';
const FRAMES_DIR = path.join(process.cwd(), 'temp_ssb4_wiiu_frames');
const OUTPUT_JSON = path.join(process.cwd(), 'ssb4-wiiu-timestamps.json');
...
async function run() {
  console.log('1. Carregando troféus do SSB4 Wii U do banco...');
  const rawTrophies = await db.collectible.findMany({
    where: { 
      type: 'TROPHY', 
      smashGameVersion: { in: ['SSB4', 'SSB4_WIIU'] } 
    },
  ...
  console.log('\n2. Extraindo frames (Crop no Top-Left do Wii U corrigido para 720p - Y=50)...');
  // Crop 720p: X=0, Y=50, W=600, H=130 (pega exatamente a barra azul de cima e evita a descrição)
  execSync(`ffmpeg -i "${VIDEO_PATH}" -vf "fps=1,crop=600:130:0:50" "${FRAMES_DIR}/frame_%05d.jpg" -y`, { stdio: 'inherit' });
  console.log('Extração concluída!');

  // 3. OCR (Multi-threading)
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    try {
      ...
      if (i % 50 === 0) console.log(`[Wii U] Progresso: ${second}s... (Lido: "${readClean}")`);
  ...
  console.log(`\nFinalizado! ${results.length} timestamps de Wii U salvos em ${OUTPUT_JSON}`);
}
