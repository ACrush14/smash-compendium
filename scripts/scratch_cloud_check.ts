...
async function main() {
  ...
  if (!f) { console.log("Cloud not found"); return; }
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Cloud" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Cloud": ${orphans.length}`);
  orphans.forEach(o => console.log(`  [${o.smashGameVersion}] "${o.name}"`));

  const cclCheck = await db.collectible.findMany({ where: { name: { contains: "Cloud" }, type: "TROPHY", smashGameVersion: "SSB4" }, select: { id: true, name: true, chronicleLinks: { select: { chronicleEntry: { select: { titleNtsc: true, consoleName: true } } } } } });
  cclCheck.forEach(c => console.log(`\n${c.name} chronicleLinks:`, c.chronicleLinks.map(l => `${l.chronicleEntry.titleNtsc} [${l.chronicleEntry.consoleName}]`)));
  await db.$disconnect();
}
