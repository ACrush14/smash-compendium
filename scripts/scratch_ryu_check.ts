...
async function main() {
  ...
  if (!f) { console.log("Ryu not found"); return; }
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Ryu" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Ryu": ${orphans.length}`);
