...
async function main() {
  ...
  const trophies = await db.collectible.findMany({ where: { name: { in: ["Mii Swordfighter", "Mii Swordfighter (Alt.)"] }, type: "TROPHY", smashGameVersion: "SSB4" }, select: { name: true, fighterId: true, videoStartSec: true, videoEndSec: true, videoStartSec2: true, videoEndSec2: true } });
