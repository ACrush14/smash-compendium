...
async function main() {
  ...
  if (!f) { console.log("Pac-Man not found"); return; }
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Pac" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Pac": ${orphans.length}`);
