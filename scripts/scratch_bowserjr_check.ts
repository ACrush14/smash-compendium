...
async function main() {
  ...
  if (!f) { console.log("Bowser Jr not found"); return; }
  ...
  const ssbbTrophy = await db.collectible.findFirst({ where: { name: { contains: "Bowser Jr" }, type: "TROPHY", smashGameVersion: "SSBB" }, select: { name: true, videoStartSec: true, videoEndSec: true, fighterId: true } });
  console.log(`\nSSBB Trophy: ${ssbbTrophy ? `"${ssbbTrophy.name}" ${ssbbTrophy.videoStartSec}-${ssbbTrophy.videoEndSec} fighterId:${ssbbTrophy.fighterId}` : "not found"}`);
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Koopaling" }, type: "TROPHY" }, select: { name: true, smashGameVersion: true, fighterId: true } });
  console.log(`\n"Koopaling" trophies: ${orphans.length}`);
  orphans.forEach(o => console.log(`  [${o.smashGameVersion}] "${o.name}" fighterId:${o.fighterId}`));

  const larry = await db.collectible.findMany({ where: { name: { in: ["Larry", "Roy", "Lemmy", "Ludwig", "Morton", "Wendy", "Iggy"] }, type: "TROPHY" }, select: { name: true, smashGameVersion: true, fighterId: true } });
  console.log(`\nKoopaling-name trophies: ${larry.length}`);
  larry.forEach(o => console.log(`  [${o.smashGameVersion}] "${o.name}" fighterId:${o.fighterId}`));
  await db.$disconnect();
}
