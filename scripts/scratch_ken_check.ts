...
async function main() {
  ...
  if (!f) { console.log("Ken not found"); return; }
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Ken" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Ken": ${orphans.length}`);
  orphans.forEach(o => console.log(`  [${o.smashGameVersion}] "${o.name}"`));

  const ccl = await db.collectible.findFirst({ where: { name: "Ken", type: "TROPHY", smashGameVersion: "SSB4" }, select: { id: true, chronicleLinks: { select: { chronicleEntry: { select: { titleNtsc: true } } } } } });
  console.log(`\nKen SSB4 trophy chronicleLinks:`, ccl?.chronicleLinks.map(c => c.chronicleEntry.titleNtsc));
  await db.$disconnect();
}
