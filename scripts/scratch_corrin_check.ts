...
async function main() {
  ...
  if (!f) { console.log("Corrin not found"); return; }
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Corrin" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Corrin": ${orphans.length}`);
  orphans.forEach(o => console.log(`  [${o.smashGameVersion}] "${o.name}"`));

  const ccl = await db.collectible.findMany({ where: { name: { contains: "Corrin" }, type: "TROPHY", smashGameVersion: "SSB4" }, select: { id: true, name: true, chronicleLinks: { select: { chronicleEntry: { select: { titleNtsc: true } } } } } });
