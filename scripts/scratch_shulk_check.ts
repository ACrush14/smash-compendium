...
async function main() {
  ...
  if (!f) { console.log("Shulk not found"); return; }
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Shulk" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Shulk": ${orphans.length}`);
