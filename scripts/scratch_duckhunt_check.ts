...
async function main() {
  ...
  if (!f) { console.log("Duck Hunt not found"); return; }
  ...
  const ssbmTrophy = await db.collectible.findFirst({ where: { name: { contains: "Duck" }, type: "TROPHY", smashGameVersion: "SSBM" }, select: { name: true, videoStartSec: true, videoEndSec: true, fighterId: true } });
  console.log(`\nSSBM Trophy: ${ssbmTrophy ? `"${ssbmTrophy.name}" ${ssbmTrophy.videoStartSec}-${ssbmTrophy.videoEndSec} fighterId:${ssbmTrophy.fighterId}` : "not found"}`);
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Duck Hunt" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Duck Hunt": ${orphans.length}`);
