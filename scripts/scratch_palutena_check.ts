...
async function main() {
  ...
  if (!f) { console.log("Palutena not found"); return; }
  ...
  const ssbbTrophy = await db.collectible.findFirst({ where: { name: "Palutena", type: "TROPHY", smashGameVersion: "SSBB" }, select: { name: true, videoStartSec: true, videoEndSec: true, fighterId: true } });
  console.log(`\nSSBB Trophy: ${ssbbTrophy ? `${ssbbTrophy.videoStartSec}-${ssbbTrophy.videoEndSec} fighterId:${ssbbTrophy.fighterId}` : "not found"}`);
  ...
  const orphans = await db.collectible.findMany({ where: { name: { contains: "Palutena" }, type: "TROPHY", fighterId: null }, select: { name: true, smashGameVersion: true } });
  console.log(`\nOrphaned trophies containing "Palutena": ${orphans.length}`);
